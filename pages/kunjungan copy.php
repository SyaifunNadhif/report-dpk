...
<style>
  /* FORCE camera modal to the very top */
  .kun-cam-backdrop{
    z-index: 2147483647 !important; /* max int, di atas semua navbar */
  }
  /* optional: kalau ada elemen lain yang bikin stacking-context aneh */
...
(() => {
  ...
  const CONTACTED = new Set(['ALM','PBD','KSS','BCN','SKT','JJA','JJJ','RES','HPR','PTP','PET','PPK','LNS']);
  const NOCONTACT = new Set(['FRD','ARA','CRA','PHD','RKS','SKP']);

  // —— state
  let latestGeo = { lat:null, lng:null, acc:null, alamat:null };
  let currentImage = null; // { img, width, height, orientation }
  let stamped = false;

  // ===== INIT: get data debitur =====
  ...
  (async () => {
    try{
      ...
      const res = await fetch(API_URL, { method:'POST', headers, body: JSON.stringify({
        type: 'get data debitur',
        no_rekening: init.no_rekening, closing_date: init.closing_date, harian_date: init.harian_date
      })});
      const j = await res.json().catch(()=> ({}));
      if(!res.ok) throw new Error(j?.message || ('HTTP '+res.status));
      const d = j?.data || {};
      // tampil
      $('#f_no_rek').value = d.no_rekening || init.no_rekening || '-';
      $('#f_bd').value = fmtRp(d.baki_debet);   $('#f_tp').value = fmtRp(d.tunggakan_pokok);
      $('#f_tb').value = fmtRp(d.tunggakan_bunga); $('#f_kolek').value = d.kolektabilitas ?? '-';
      $('#f_hm').value = (d.hari_menunggak ?? '-') + (d.hari_menunggak!=null?' hari':'');
      // simpan raw untuk submit
      $('#f_no_rek').dataset.raw = d.no_rekening || init.no_rekening || '';
      $('#f_bd').dataset.raw = d.baki_debet || 0; $('#f_tp').dataset.raw = d.tunggakan_pokok || 0;
      $('#f_tb').dataset.raw = d.tunggakan_bunga || 0; $('#f_kolek').dataset.raw = d.kolektabilitas || '';
      $('#f_hm').dataset.raw = d.hari_menunggak || 0;
      showStatus('Data debitur berhasil dimuat.', true);
      setTimeout(hideStatus, 900);
    }catch(e){ showStatus(e?.message || 'Gagal memuat data debitur'); }
  })();

  // ===== status UI
  function showStatus(msg, ok=false){
    const el=$('#kun-status'); el.textContent=msg; el.classList.remove('hidden','ok','error'); el.classList.add(ok?'ok':'error');
  }
  function hideStatus(){ $('#kun-status')?.classList.add('hidden'); }

  // ===== geolocate + reverse geocode
  function fmtCoord(v){ return (v==null || isNaN(v)) ? '' : Number(v).toFixed(6); }
  function updateTimeBox(){ /* keep WIB time for stamping */ }

  async function resolveAddress(lat, lng){
    $('#alamat_gps').value = 'Mencari alamat…'; latestGeo.alamat=null;

    function composeOSM(addr){
      if (!addr) return null;
      const parts=[addr.road,addr.pedestrian,addr.cycleway,addr.footway,addr.neighbourhood,addr.suburb,addr.village,addr.town,addr.city,addr.state,addr.postcode,addr.country].filter(Boolean);
    ...
    try{
      ...
      for(const z of zooms){
        ...
        if(r.ok){ const d=await r.json(); const name=d.display_name||composeOSM(d.address); if(name){ latestGeo.alamat=name; $('#alamat_gps').value=name; return name; } }
      }
    }catch{}
    try{
      ...
      const r=await fetch(u); if(r.ok){ const d=await r.json(); const name=[d.locality,d.city,d.principalSubdivision,d.countryName].filter(Boolean).join(', ')||d.plusCode||null; if(name){ latestGeo.alamat=name; $('#alamat_gps').value=name; return name; } }
    }catch{}
    const approx=`Sekitar ${Number(lat).toFixed(5)}, ${Number(lng).toFixed(5)}`; latestGeo.alamat=approx; $('#alamat_gps').value=approx; return approx;
  }

  $('#btnLoc').addEventListener('click', ()=>{
    if(!navigator.geolocation){ showStatus('Geolocation tidak didukung'); return; }
    navigator.geolocation.getCurrentPosition(async pos=>{
      ...
      if(currentImage) drawWatermark(); // refresh cap di foto
      showStatus('Koordinat diambil.', true); setTimeout(hideStatus, 900);
    }, ()=> showStatus('Izin lokasi ditolak / timeout'), { enableHighAccuracy:true, timeout:10000, maximumAge:0 });
  });

  // ===== foto: upload + kamera
  const previewBox = $('.kun-preview');
  function drawImageToCanvas(img){
    const maxW = Math.max(320, previewBox.clientWidth || 320);
  ...
  function wrapText(ctx, text, maxWidth){
    ...
    if(lines.length===2 && ctx.measureText(lines[1]).width>maxWidth){ let s=lines[1]; while(s.length>3 && ctx.measureText(s+'…').width>maxWidth){ s=s.slice(0,-1); } lines[1]=s+'…'; }
    return lines;
  }
  function drawWatermark(){
    ...
    const accText = (latestGeo.acc!=null && !Number.isNaN(latestGeo.acc)) ? `Akurasi ±${Math.round(latestGeo.acc)} m` : '';
    const addrText = latestGeo.alamat ? String(latestGeo.alamat) : '';
    ...
    const addrLines = wrapText(ctx, addrText, Math.min(maxTxtW, 460));
    const lines = [ nowText, geoText + (accText?`  •  ${accText}`:'') ].concat(addrLines);
  ...
  $('#fileFoto').addEventListener('change', async ()=>{
    ...
    const img = new Image();
    img.onload = async ()=>{ currentImage={img}; if(!$('#lat').value || !$('#lng').value){ $('#btnLoc').click(); await new Promise(r=>setTimeout(r,400)); } drawWatermark(); URL.revokeObjectURL(url); };
    img.src = url;
  });
  ...
  async function startCam(deviceId){
    if(!navigator.mediaDevices?.getUserMedia){ showStatus('Browser tidak mendukung kamera'); return false; }
    try{
      ...
      return true;
    }catch(e){ showStatus('Gagal membuka kamera'); return false; }
  }
  $('#btnCamera').addEventListener('click', async ()=>{
    const pGeo = new Promise(res=>{
      if(!$('#lat').value || !$('#lng').value) $('#btnLoc').click();
      setTimeout(res, 200);
    });
  ...
  // kontrol PTP & status kunjungan
  function updateFromKode(){
    const kode = $('#kode_tindakan').value;
    const isPTP = (kode==='PTP');
    $('#nominal_janji_bayar').disabled = !isPTP;
    $('#tanggal_janji_bayar').disabled = !isPTP;
    if(!isPTP){ $('#nominal_janji_bayar').value=''; $('#tanggal_janji_bayar').value=''; }
    if(CONTACTED.has(kode)) $('#status_kunjungan').value='1';
    else if(NOCONTACT.has(kode)) $('#status_kunjungan').value='0';
    else $('#status_kunjungan').value='';
  }
  ...
  $('#btnSimpan').addEventListener('click', async ()=>{
    try{
      ...
      const payload = {
        type: 'create kunjunan petugas',
        no_rekening: ($('#f_no_rek').dataset.raw || '').trim(),
        baki_debet: Number($('#f_bd').dataset.raw || 0),
        tunggakan_pokok: Number($('#f_tp').dataset.raw || 0),
        tunggakan_bunga: Number($('#f_tb').dataset.raw || 0),
        kolektabilitas: ($('#f_kolek').dataset.raw || ''),
        hari_menunggak: Number($('#f_hm').dataset.raw || 0),

        kode_tindakan: $('#kode_tindakan').value.trim(),
        jenis_tindakan: $('#jenis_tindakan').value.trim(),
        lokasi_tindakan: $('#lokasi_tindakan').value.trim(),
        orang_ditemui: $('#orang_ditemui').value.trim(),

        nominal_janji_bayar: ($('#kode_tindakan').value==='PTP' && $('#nominal_janji_bayar').value) ? Number($('#nominal_janji_bayar').value) : 0,
        tanggal_janji_bayar: ($('#kode_tindakan').value==='PTP') ? ($('#tanggal_janji_bayar').value || '') : '',

        status_kunjungan: $('#status_kunjungan').value.trim(),
        keterangan: $('#keterangan').value.trim(),

        nama_foto: $('#nama_foto').value.trim(),
        alamat_gps: $('#alamat_gps').value.trim(),
        koordinat: ($('#lat').value && $('#lng').value) ? `${$('#lat').value}, ${$('#lng').value}` : '',
        created: $('#created').value || nowWIB()
      };

      if(!payload.no_rekening) throw new Error('No. rekening kosong');
      if(!payload.kode_tindakan) throw new Error('Pilih kode tindakan');
      if(!payload.jenis_tindakan) throw new Error('Pilih jenis tindakan');

      const res = await fetch(API_URL, { method:'POST', headers, body: JSON.stringify(payload) });
      const j = await res.json().catch(()=> ({}));
      if(!res.ok) throw new Error(j?.message || ('HTTP '+res.status));

      showStatus('Kunjungan berhasil dibuat.', true);
      // setTimeout(()=> history.back(), 800);
    }catch(e){ showStatus(e?.message || 'Gagal menyimpan kunjungan'); }
  });
})();
</script>
