<div class="max-w-[1920px] mx-auto px-4 py-6 h-screen flex flex-col font-sans text-gray-800 bg-gray-50">
  ...
  <div class="mb-4">
    <h1 class="text-2xl font-bold flex items-center gap-2 text-gray-800">
      ...
      <span>Monitoring Repayment Rate (RR)</span>
    </h1>
  </div>

  <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-4">
    
    <div id="summaryWrapRR" class="flex flex-wrap items-center gap-2 text-sm animate-fade-in hidden order-2 lg:order-1">
      <span class="pill pill-blue">Target: <b id="sum_os_target">0</b></span>
      <span class="pill pill-green">Lancar: <b id="sum_os_lancar">0</b></span>
      <span class="pill pill-red">Ditagih: <b id="sum_os_tagih">0</b></span>
      <span class="pill pill-purple">Recovery: <b id="sum_persen">0%</b></span>
    </div>

    <form id="formFilterRR" class="flex flex-wrap items-end gap-2 order-1 lg:order-2 ml-auto bg-white p-2 rounded-lg border border-gray-200 shadow-sm">
      <div class="field">
        <label class="lbl">Closing (M-1)</label>
        <input type="date" id="closing_date" class="inp py-1" required>
      </div>
      <div class="field">
        <label class="lbl">Actual (Harian)</label>
        <input type="date" id="harian_date" class="inp py-1" required>
      </div>
      <div class="field">
        <label class="lbl">Kantor Cabang</label>
        <select id="opt_kantor" class="inp py-1 min-w-[150px]">
            <option value="">Loading...</option>
        </select>
      </div>
      <button type="submit" id="btnFilterRR" class="h-[34px] px-4 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-bold uppercase tracking-wider transition shadow-sm flex items-center gap-1">
        Tampilkan
      </button>
    </form>
  </div>

  <div class="flex-1 min-h-0 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm relative">
    
    <div id="loadingRR" class="hidden absolute inset-0 bg-white/80 z-50 flex flex-col items-center justify-center text-sm text-blue-600 font-medium">
    ...
    <div class="h-full overflow-auto custom-scrollbar">
      <table class="min-w-full text-xs text-center border-separate border-spacing-0 text-gray-600">
        <thead class="uppercase bg-gray-100 text-gray-600 font-bold sticky top-0 z-30 shadow-sm">
          <tr>
            <th rowspan="2" class="px-2 py-2 border-b border-r bg-gray-100 sticky left-0 z-20 min-w-[60px]">TGL</th>
        ...
        <tbody id="bodyRR" class="divide-y divide-gray-100 bg-white"></tbody>
        <tfoot id="footRR" class="bg-gray-800 text-white font-bold sticky bottom-0 z-30 text-[11px]"></tfoot>
      </table>
    </div>
  </div>
</div>
...
<div id="modalDetailRR" class="fixed inset-0 hidden z-[9999] flex items-center justify-center p-4">
  <div class="absolute inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity" onclick="closeModalRR()"></div>
  <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-7xl max-h-[90vh] flex flex-col animate-scale-up overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b bg-white">
      <div>
        ...
        <p class="text-xs text-gray-500 mt-0.5 ml-3" id="modalSubTitleRR">...</p>
      </div>
      <div class="flex items-center gap-3">
          ...
          <button onclick="closeModalRR()" class="text-gray-300 hover:text-red-500 transition text-2xl leading-none">&times;</button>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-auto bg-gray-50 relative min-h-[300px]">
      <div id="loadingModalRR" class="hidden absolute inset-0 bg-white/90 z-20 flex flex-col items-center justify-center text-blue-600">
         ...
         <span class="text-xs font-bold uppercase mt-2">Memuat Detail...</span>
      </div>
      
      <table class="w-full text-xs text-left text-gray-600 border-separate border-spacing-0" id="tableExportRR">
        <thead id="headModalRR" class="text-xs text-gray-500 uppercase bg-gray-100 sticky top-0 shadow-sm z-10"></thead>
        <tbody id="bodyModalRR" class="divide-y divide-gray-200 bg-white"></tbody>
      </table>
    </div>

    <div class="px-6 py-3 border-t bg-white flex justify-between items-center">
      <span class="text-xs font-bold text-gray-400" id="pageInfoRR">0 Data</span>
      <div class="flex gap-2">
          <button id="btnPrevRR" onclick="changePageDetail(-1)" class="px-3 py-1.5 bg-white border border-gray-300 rounded text-xs font-medium hover:bg-gray-50 disabled:opacity-50 transition shadow-sm">Prev</button>
          <button id="btnNextRR" onclick="changePageDetail(1)" class="px-3 py-1.5 bg-white border border-gray-300 rounded text-xs font-medium hover:bg-gray-50 disabled:opacity-50 transition shadow-sm">Next</button>
      </div>
    </div>
  </div>
</div>
...
<style>
  .inp { border:1px solid #cbd5e1; border-radius:6px; padding:0 8px; font-size:13px; background:#fff; transition:all 0.2s; outline:none; color: #334155; }
  ...
  /* Pills */
  .pill { display:inline-flex; align-items:center; padding:4px 10px; border-radius:99px; border:1px solid; font-size:11px; font-weight:600; box-shadow:0 1px 2px rgba(0,0,0,0.05); }
  ...
  @media (max-width: 768px) {
      #formFilterRR { width: 100%; justify-content: space-between; }
      .field { flex: 1; }
  }
</style>
...
<script>
  ...
  // Wrapper Fetch agar aman dari error HTML Server
  const apiCall = async (url, opt = {}) => {
      ...
      try {
          ...
          throw new Error("Gagal parsing JSON. Cek Console.");
      }
  };
  ...
  async function fetchRekapRR(){
    ...
    tb.innerHTML = `<tr><td colspan="11" class="py-12 text-center text-gray-400 italic">Memuat data...</td></tr>`;
    ft.innerHTML = '';
    ...
    try {
        ...
        renderTableRR(res.json.data.data || [], res.json.data.grand_total);
        
    } catch(err) {
        if(err.name !== 'AbortError') {
            console.error(err);
            tb.innerHTML=`<tr><td colspan="11" class="py-12 text-center text-red-500 font-medium">Error: ${err.message}</td></tr>`;
        }
  ...
  function renderTableRR(rows, gt) {
      ...
      tb.innerHTML = '';
      if(rows.length === 0){ tb.innerHTML = `<tr><td colspan="11" class="py-12 text-center text-gray-400">Tidak ada data.</td></tr>`; return; }
      ...
      rows.forEach(r => {
          ...
          const clkLns = `<a href="javascript:void(0)" onclick="initModalLunas(${r.tgl})" class="font-bold text-gray-600 hover:text-blue-600 hover:underline cursor-pointer">${fmt(r.lunas_os)}</a>`;

          h += `
            <tr class="hover:bg-blue-50 transition border-b group h-[34px] ${bg}">
                <td class="px-2 sticky left-0 bg-white group-hover:bg-blue-50 border-r font-bold text-gray-700 text-center">${r.tgl}</td>
                <td class="px-2 border-r text-right text-gray-500">${fmt(r.target_noa)}</td>
                <td class="px-2 border-r text-right bg-blue-50/30">${clkAll}</td>
                <td class="px-2 border-r text-right text-gray-500">${fmt(r.lancar_noa)}</td>
                <td class="px-2 border-r text-right bg-green-50/30">${clkLcr}</td>
                <td class="px-2 border-r text-right text-gray-500">${fmt(r.macet_noa)}</td>
                <td class="px-2 border-r text-right bg-red-50/30">${clkTgh}</td>
                <td class="px-2 border-r text-right text-gray-400 bg-gray-50">${fmt(r.lunas_noa)}</td>
                <td class="px-2 border-r text-right bg-gray-50">${clkLns}</td>
                <td class="px-2 border-r text-right text-gray-400">${fmt(r.angsuran)}</td>
      ...
      if(gt) {
        ft.innerHTML = `
            <tr class="h-[40px]">
                <td class="px-2 sticky left-0 bg-gray-800 border-r border-gray-600 text-center">TOTAL</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono">${fmt(gt.target_noa)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-blue-300">${fmt(gt.target_os)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-gray-400">${fmt(gt.lancar_noa)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-green-400">${fmt(gt.lancar_os)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-gray-400">${fmt(gt.macet_noa)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-red-400">${fmt(gt.macet_os)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-gray-400 bg-gray-700">${fmt(gt.lunas_noa)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-gray-400">${fmt(gt.lunas_os)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-gray-400">${fmt(gt.angsuran)}</td>
                <td class="border-r border-gray-600 px-2 text-right font-mono text-purple-300">${fmt(gt.total_bayar)}</td>
  ...
  function initModalDetail(tgl, status) {
      ...
      document.getElementById('headModalRR').innerHTML = `
          <tr>
            <th class="px-4 py-3 bg-gray-100 border-b">No Rekening</th>
            <th class="px-4 py-3 bg-gray-100 border-b">Nama Nasabah</th>
            <th class="px-4 py-3 bg-gray-100 border-b text-center text-blue-700">Nama AO</th>
            <th class="px-4 py-3 bg-gray-100 border-b text-center">Tgl JT</th>
            <th class="px-4 py-3 bg-gray-100 border-b text-right">Plafond</th>
            ...
            <th class="px-4 py-3 bg-gray-100 border-b text-center">DPD</th>
            <th class="px-4 py-3 bg-gray-100 border-b text-center">Status</th>
          </tr>`;
  ...
  function initModalLunas(tgl) {
      ...
      document.getElementById('headModalRR').innerHTML = `
          <tr>
            <th class="px-4 py-3 bg-gray-100 border-b">Nama Nasabah</th>
            <th class="px-4 py-3 bg-gray-100 border-b text-center text-blue-700">Nama AO</th>
            <th class="px-4 py-3 bg-gray-100 border-b">Rek Lama (Lunas)</th>
            <th class="px-4 py-3 bg-gray-100 border-b border-l text-right">Plafond Lama</th>
            <th class="px-4 py-3 bg-blue-50 text-blue-700 border-b border-l text-right">OS M-1</th>
            <th class="px-4 py-3 bg-gray-100 border-b border-l text-center">Status</th>
  ...
  async function loadDetailPage(page) {
      ...
      try {
          ...
          if(list.length === 0) {
              tb.innerHTML = `<tr><td colspan="10" class="py-12 text-center text-gray-400 italic">Tidak ada data detail.</td></tr>`;
              ...
              list.forEach(r => {
                  ...
                  if(currentMode === 'NORMAL') {
                      let badge = `<span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">${r.status_ket}</span>`;
                      ...
                      h += `<tr class="hover:bg-gray-50 transition border-b">
                            <td class="px-4 py-2 font-mono text-xs text-gray-600">${r.no_rekening}</td>
                            <td class="px-4 py-2 font-medium text-gray-800 truncate max-w-[200px]" title="${r.nama_nasabah}">${r.nama_nasabah}</td>
                            <td class="px-4 py-2 text-center font-bold text-xs text-blue-700 bg-blue-50/10">${aoName}</td>
                            <td class="px-4 py-2 text-center font-mono text-xs text-gray-500">${r.tgl_jatuh_tempo||'-'}</td>
                            <td class="px-4 py-2 text-right text-gray-500 text-xs">${fmt(r.jml_pinjaman)}</td>
                            ...
                            <td class="px-4 py-2 text-center font-bold text-gray-700">${r.dpd_curr}</td>
                      ...
                      h += `<tr class="hover:bg-gray-50 transition border-b">
                            <td class="px-4 py-2 font-medium text-gray-800 truncate max-w-[200px]">
                                ${r.nama_nasabah}
                                <div class="text-[10px] text-gray-400 font-mono">ID: ${r.nasabah_id}</div>
                            </td>
                            <td class="px-4 py-2 text-center font-bold text-xs text-blue-700 bg-blue-50/10">${aoName}</td>
                            <td class="px-4 py-2 font-mono text-xs text-gray-600">${r.no_rekening}</td>
                            <td class="px-4 py-2 text-right text-gray-600 bg-gray-50 border-l border-gray-200">${fmt(r.plafon_lama)}</td>
