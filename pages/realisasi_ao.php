    ...
    <div id="top50ScrollerInner" class="h-full overflow-auto custom-scrollbar">
      <table id="tabelTop50" class="w-full text-xs text-left text-slate-700 border-separate border-spacing-0">
        <thead class="uppercase bg-slate-100 text-slate-600 font-bold sticky top-0 z-30 shadow-sm text-[10px] md:text-xs tracking-wider">
          <tr>
            <th class="px-3 md:px-4 py-3 sticky left-0 z-40 bg-slate-100 border-b border-slate-200 text-center w-[50px]">NO</th>
            <th class="px-3 md:px-4 py-3 bg-slate-100 border-b border-slate-200 text-center w-[80px]">CABANG</th>
            <th class="px-3 md:px-4 py-3 bg-slate-100 border-b border-slate-200 text-center w-[120px]">KODE AO</th>
            <th class="px-3 md:px-4 py-3 bg-slate-100 border-b border-slate-200 w-auto">NAMA AO (MARKETING)</th>
            <th class="px-3 md:px-4 py-3 bg-slate-100 border-b border-slate-200 text-center w-[100px]">TOTAL NOA</th>
            <th class="px-3 md:px-4 py-3 bg-emerald-50 text-emerald-800 border-b border-emerald-200 text-right w-[200px]">TOTAL REALISASI</th>
          </tr>
        </thead>
...
<script>
  ...
  window.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load Tanggal Murni JS (Seperti Request Sebelumnya)
    const now = new Date();
  ...
  function fetchTop50Data() {
    ...
    // Logika Judul Konsolidasi vs Cabang
    ...
    // Payload dikirim ke Router API Lu
    const payload = { 
        type: "top realisasi", 
    ...
    data.forEach((row, index) => {
      html += `
        <tr class="border-b border-slate-100 transition hover:bg-blue-50/50 group h-[40px]">
          <td class="px-3 md:px-4 py-2 text-center sticky left-0 z-20 bg-white group-hover:bg-blue-50/50 font-mono text-xs text-slate-500">
            ${index + 1}
          </td>
          <td class="px-3 md:px-4 py-2 text-center font-mono text-xs text-slate-500">
            ${row.kode_cabang || "-"}
          </td>
          <td class="px-3 md:px-4 py-2 text-center font-mono text-xs font-bold text-slate-700">
            ${row.kode_ao || "-"}
          </td>
          <td class="px-3 md:px-4 py-2 font-semibold text-blue-700 truncate">
            ${row.nama_ao || "-"}
          </td>
          <td class="px-3 md:px-4 py-2 text-center font-bold text-slate-700">
            ${new Intl.NumberFormat('id-ID').format(row.total_noa || 0)} <span class="text-[9px] text-slate-400 font-normal ml-1">Debitur</span>
          </td>
          <td class="px-3 md:px-4 py-2 text-right font-bold text-emerald-800 bg-emerald-50/30">
