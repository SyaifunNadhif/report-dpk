...
/*********************** REALISASI (akun baru) ***********************/
/** REALISASI: akun baru (tampil data harian saja, filter tgl_realisasi > closing & <= harian)
 *  - Wajib: $kc spesifik (bukan konsolidasi)
 *  - Cepat: tanpa join closing/transaksi/ckpn
 */
public function getMigrasiBucketDetailRealisasi(string $closing, string $harian, string $kc, ?string $tbFilter=null)
{
  // window created untuk snapshot harian
  [$dsH,$deH] = $this->dayRange($harian);

  // NOTE performance:
  //  Pastikan ada index:
  //   - nominatif(created, kode_cabang)
  //   - nominatif(tgl_realisasi, kode_cabang)
  //   - nominatif(hari_menunggak)
  //  ref_dpd_bucket kecil → join aman.
  ...
  $sql = "
    SELECT
      ...
      nh.no_rekening,
      nh.nama_nasabah,
      NULL                AS alamat,         -- biar aman lintas DB; isi NULL sesuai permintaan
      'REALISASI'         AS from_bucket,
      rb2.dpd_code        AS to_bucket,
      nh.baki_debet       AS baki_debet,     -- OS actual
      nh.kolektibilitas,
      nh.kode_produk,
      nh.tunggakan_pokok,
      nh.tunggakan_bunga,
      ...
      /* field-field non-relevan untuk realisasi → NULL */
...
public function getMigrasiBucketDetailActual(string $closing, string $harian, ?string $kc, string $fb, string $tb){
  // WAJIB cabang spesifik; konsolidasi akan sangat berat
  if ($kc===null || $kc==='000') {
    return sendResponse(400, "Pilih kode_kantor spesifik (CHAR(3), mis. '004') untuk detail ACTUAL.");
  }
  // normalisasi ke CHAR(3) tanpa mengubah tipe
  ...
  $sql = "
    SELECT
      ...
      h.baki_debet  AS baki_debet,       -- OS actual
      h.kolektibilitas,
      h.kode_produk,
      h.tunggakan_pokok,
      h.tunggakan_bunga,
    ...
    JOIN nominatif h
     ...
     AND h.kode_cabang = :kcH    -- CHAR(3) langsung
...
public function getMigrasiBucketDetailOLunas(string $closing, string $harian, ?string $kc, string $fromBucket){
  ...
  $sql = "
    SELECT
      ...
      c.baki_debet  AS baki_debet,        -- pakai OS M-1
      c.kolektibilitas,
      c.kode_produk,
      c.tunggakan_pokok,
      c.tunggakan_bunga,
...
private function attachCkpnAndCast(array $rows, string $harian, string $closing, ?string $kc, string $mode){
  ...
  $ckpnM1Map = [];
  if ($mode==='o_lunas' && $accs) {
    $ckpnM1Map = $this->fetchSnapCkpnMap($closing, $kc, $accs);
    if (count($ckpnM1Map) < count($accs)) {
      $pdMapC = $this->loadPdMap($closing);
      $LGDC   = $this->loadGlobalLGD($closing);
      foreach ($rows as $r){
        $acc = $r['no_rekening'];
        if (isset($ckpnM1Map[$acc])) continue;
        $fake = [
          'no_rekening'=>$acc,
        ...
        $ckpnM1Map[$acc] = $this->computeCkpnForRow($fake, $closing, $pdMapC, $LGDC, $indivMap, $restruk, null);
      }
    }
  }
  ...
  foreach ($rows as &$r){
    if ($mode==='o_lunas'){
      $r['ckpn_actual'] = 0;
      $r['ckpn_m1']     = (int)round($ckpnM1Map[$r['no_rekening']] ?? 0);
    } else {
      $snap = $snapMapH[$r['no_rekening']] ?? null;
